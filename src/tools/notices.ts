...
export function registerNoticeTools(server: McpServer, ctx: ToolCtx): void {
  ...
  server.registerTool(
    "search_notices",
    {
      title: "공지 검색",
      description:
        "who-will-notify-mcp 서비스의 학과 공지 게시판에서 질문과 관련된 공지를 키워드로 검색해 요약(제목·분류·게시일·발췌)을 점수순으로 반환합니다. " +
  ...
  server.registerTool(
    "list_notices",
    {
      title: "공지 목록",
      description:
        "who-will-notify-mcp 서비스에서 학과 공지를 최신순으로 나열합니다(요약만). 분류(category)나 학기(semester)로 필터링할 수 있습니다. " +
  ...
  server.registerTool(
    "get_notice",
    {
      title: "공지 상세",
      description:
        "who-will-notify-mcp 서비스에서 id 로 공지 한 건의 전체 내용(본문·개설과목·일정 포함)을 반환합니다. " +
